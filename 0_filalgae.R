...
fil_algae_timeseries <- macrophyte_ntl %>%
  ...
  # 2023 data are swapped in the dataset
  mutate(
    fil_algae_tmp = if_else(year4 == 2023, plant_wt_sum, fil_algae_sum),
    plant_wt_tmp = if_else(year4 == 2023, fil_algae_sum, plant_wt_sum)
  ) %>%
  select(-fil_algae_sum, -plant_wt_sum) %>%
  rename(fil_algae_sum = fil_algae_tmp, plant_wt_sum = plant_wt_tmp)
